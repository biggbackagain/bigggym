<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Crear Nuevo Plan de Membresía') }}
        </h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('admin.memberships.store') }}">
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="duration_days" class="block mt-1 w-full" type="number" name="duration_days" :value="old('duration_days')" required />
                            <x-input-error :messages="$errors->get('duration_days')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="price_general" :value="__('Precio General ($)')" />
                            <x-text-input id="price_general" class="block mt-1 w-full" type="number" name="price_general" :value="old('price_general')" step="0.01" required />
                            <x-input-error :messages="$errors->get('price_general')" class="mt-2" />
                        </div>
                        
                        <div class="mt-4">
                            <x-input-label for="price_student" :value="__('Precio Estudiante ($)')" />
                            <x-text-input id="price_student" class="block mt-1 w-full" type="number" name="price_student" :value="old('price_student')" step="0.01" required />
